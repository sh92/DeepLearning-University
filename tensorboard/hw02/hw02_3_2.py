...
with tf.name_scope('Layer1'):
   W1 = tf.get_variable("W1", shape=[19, l1_class],
...
with tf.name_scope('Layer2'):
   W2 = tf.get_variable("W2", shape=[l1_class, l2_class],
                                initializer=tf.contrib.layers.xavier_initializer())
...
with tf.name_scope('Layer3'):
   W3 = tf.get_variable("W3", shape=[l2_class, l3_class],
                               initializer=tf.contrib.layers.xavier_initializer())
   b3 = tf.Variable(tf.random_normal([l3_class]), name='bias3')
...
with tf.name_scope('Layer4'):
   W4 = tf.get_variable("W4", shape=[l3_class, l4_class],
                               initializer=tf.contrib.layers.xavier_initializer())
   b4 = tf.Variable(tf.random_normal([l4_class]), name='bias4')
...
with tf.name_scope('Layer5'):
   W5 = tf.get_variable("W5", shape=[l4_class, nb_class],
                                initializer=tf.contrib.layers.xavier_initializer())
   b5 = tf.Variable(tf.random_normal([nb_class]), name='bias5')
...
with tf.Session() as sess:
   ...
   writer = tf.summary.FileWriter("./board2_3_2");
   total_batch = int( len(train_data)/ batch_size)
   for epoch in range(training_epochs):
      avg_train_cost = 0
      avg_train_acc = 0
      for i in range(total_batch):
         feed_dict = {X: train_data_x, Y: train_data_y, keep_prob: 0.7 }
      ...
      test_a = sess.run(accuracy, feed_dict={X:test_data_x, Y:test_data_y, keep_prob: 1})
