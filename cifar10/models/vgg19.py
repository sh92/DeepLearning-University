...
class Model:
    ...
    def _build_net(self):
	    ...
	    self.pool4 = tf.nn.dropout(self.pool4, self.dropout)
	    ...
	    self.fc8 = self.fc_layer(self.relu7, 4096, 1000, "fc8")
            ...
            #self.fc8 = tf.contrib.layers.flatten(self.pool3)
            #self.fc8 = tf.layers.dense(self.fc8, 1000)
	    #self.fc8 = tf.nn.dropout(self.fc8, self.dropout)

            self.flat = tf.reshape(self.fc8, [self.batch_size,10])
            self.logits = tf.layers.dense(inputs=self.fc8, units=self.num_classes)
    ...
    def set_size(self, num_x, num_y, num_classes, batch_size=100):
        self.num_x = num_x 
        self.num_y = num_y
